...
class MyLocalDatabase {
  ...
  openMyDatabase() async {
    myDatabase = await openDatabase("My_Todo", version: 1,
        onCreate: (Database db, int ver) async {
      await db.execute(
          "CREATE TABLE Tasks (id INTEGER PRIMARY KEY, value TEXT, isDone INTEGER)");
  ...
  addTask(Task task) async {
    await myDatabase.transaction((txn) async {
      txn.rawInsert(
          'INSERT INTO Tasks(value, isDone) VALUES("${task.data}", ${0})');
    });
  }

  Future<List<Task>> getTasks() async {
    List<Map<String, dynamic>> list =
        await myDatabase.rawQuery('SELECT * FROM Tasks');
    List<Task> myTasks = [];
